const config = require('./config');  // jwt secret and port
...
app.get('/api/auth/', authenticate, function (req, res) {
    res.status(200);
    res.json({user: req.decoded.user});
});
...
// user won, increment their score by 1
// requires authentication
app.post('/api/win/', authenticate, function (req, res) {
    ...
    // validate inputs
    let badkey = validate(req.body.user, ['id']);
    if (badkey !== undefined) {
        res.status(400);
        res.json({error: `value of ${badkey} is not alphanumeric, but should be.`});
        return;
    ...
    if (!Number.isInteger(score)) {
        res.status(400);
        res.json({error: `value of score is not an integer, but should be.`});
    }
